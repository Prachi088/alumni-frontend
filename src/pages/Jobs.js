  ...
  return (
    <div style={styles.container}>
      <div style={styles.content}>
        ...
        <input type="text" placeholder="Search jobs..." value={search} onChange={(e) => setSearch(e.target.value)} style={styles.search} />
        {role === "alumni" && (
          ...
          {filteredJobs.length > 0 ? (
            filteredJobs.map(job => (
              <div key={job.id} style={styles.card}>
                ...
                {role !== "alumni" && (
                  <button style={job.applied ? styles.disabledBtn : styles.applyBtn} onClick={() => handleApply(job.id)} disabled={job.applied}>
...
const styles = {
  ...
  search:     { padding:"12px 16px", width:"100%", maxWidth:"400px", marginBottom:"28px", borderRadius:"10px", border:"1px solid rgba(99,102,241,0.2)", background:"#fff", color:"#1e293b", fontSize:"14px", outline:"none", boxShadow:"0 2px 8px rgba(0,0,0,0.06)" },
  ...
  input:      { padding:"12px 16px", borderRadius:"8px", border:"1px solid rgba(99,102,241,0.2)", background:"#f8fafc", color:"#1e293b", fontSize:"14px", outline:"none", flex:1, minWidth:"200px" },
  addBtn:     { background:"linear-gradient(135deg, #10b981, #34d399)", color:"#fff", border:"none", padding:"12px 24px", borderRadius:"8px", cursor:"pointer", fontWeight:"700", fontSize:"14px" },
  grid:       { display:"flex", gap:"24px", flexWrap:"wrap" },
  card:       { background:"linear-gradient(135deg, #fff 0%, #f9fafb 100%)", padding:"24px", borderRadius:"14px", boxShadow:"0 4px 20px rgba(0,0,0,0.06)", width:"280px", border:"1px solid rgba(99,102,241,0.08)" },
  title:      { fontSize:"16px", fontWeight:"700", margin:"0 0 12px", color:"#6366f1" },
  text:       { fontSize:"14px", color:"#64748b", margin:"8px 0" },
  applyBtn:   { marginTop:"16px", padding:"10px 16px", width:"100%", background:"linear-gradient(135deg, #6366f1, #8b5cf6)", color:"#fff", border:"none", borderRadius:"8px", cursor:"pointer", fontWeight:"700", fontSize:"13px" },
  ...
  deleteBtn:  { marginTop:"16px", padding:"10px 16px", width:"100%", background:"linear-gradient(135deg, #ef4444, #f87171)", color:"#fff", border:"none", borderRadius:"8px", cursor:"pointer", fontWeight:"700", fontSize:"13px" },
};
